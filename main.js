 ...
 */
async function loadScene(tag, data, html, regenerateHtml = true) {
  if (currentScene) {
    if (currentScene.returnable) {
      sceneStack.push({
        scene: currentScene,
        html: generatedHtml,
...
async function back() {
  if (sceneStack.length > 0) {
    ...
    await loadScene(scene.tag, scene.data, scene.generatedHtml, scene.regenerateHtml)
  }
}
